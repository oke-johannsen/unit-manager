...
if (Meteor.isServer) {
  ...
  Meteor.methods({
    'skills.create': (payload) => {
      const { name, trainers, link, color, type, designation } = payload
      Meteor.call('logging.create', {
        key: 'skills.create',
        before: null,
        after: { name, trainers, link, color, type, designation },
        userId: Meteor.user()?._id,
      })
      SkillsCollection.insert({ name, trainers, link, color, type, designation }, (err, res) => {
        if (!err) {
          return true
        } else {
    ...
    'skills.update': (id, payload) => {
      ...
      SkillsCollection.update(id, { $set: { name, trainers, link, color, type, designation } }, (err, res) => {
        if (!err) {
          return true
        } else {
