import { Badge, Button, Col, DatePicker, Divider, Form, Input, InputNumber, Row, Segmented, Select, Tag } from 'antd'
...
const UserForm = ({ userId, closeModal, forms, setForms, submitForms }) => {
  ...
  const [segementedValue, setSegmentedValue] = useState('form')
  const securityClearance = Number(Meteor.user()?.profile?.securityClearance)
  return (
    <Row>
      <Col span={24}>
        <Segmented
          value={segementedValue}
      ...
      <Col span={24}>
        ...
        {segementedValue === 'dashboard' && <UserDashboardComponent userProp={user} />}
      </Col>
    </Row>
  )
}
